...
class Navigator():
    ...
    def spin(self):
        ...
        if self.mode == "NAVIGATE":
            print("Navigate")
            if self.distance < self.dist_low_lim:
                self.mode = "IDLE"
    ...
    def navigate(self):
        command = msg.Twist()
        command.linear.x = self.throttle
        command.angular.z = self.pan_angle
...
pub = rospy.Publisher("/segway/assisted_teleop/cmd_vel", msg.Twist, queue_size=1)
